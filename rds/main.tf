...
resource "aws_db_instance" "database_instance" {
  ...
  skip_final_snapshot    = true #this means when we use terraform destroy, it will not create a final snapshot
  availability_zone      = var.availability_zone_1
  identifier             = var.database_instance_identifier
  snapshot_identifier    = var.aws_db_snapshot.latest_db_snapshot.id
  db_subnet_group_name   = var.aws_db_subnet_group.database_subnet_group.name
